...
import {
    disconnectSimulator, reconfigCharger,
    trigInitState,
    trigSetTestReady,
    trigStartEndFromWs, trigTestDone,
    updateSimulatorData
} from "./simulatorInfoState";
...
const webSocketState = handleActions(
    {
        ...
        [OPEN_WEBSOCKET]: (state, action) => {
            ...
            const socket = new WebSocket(`${window.location.protocol === 'https:' ? 'wss' : 'ws'}://${window.location.host}${process.env.REACT_APP_WEB_SOCKET_URL}`);
            ...
            socket.onmessage = e => {
                ...
                if(isJsonString(e.data)) {
                    ...
                    if(data.DataType === "event") {
                        if (data.hasOwnProperty("testSuccess")) {
                            dispatch(reconfigCharger(parseInt(data.simulatorID), 0));
                            dispatch(trigTestDone(data));
                        } else {
                    ...
                    if(data.DataType === "setInitState") {
                        if(localStorage.getItem('uuid') !== data.uuid) dispatch(trigInitState(data));
                        return;
                    }
