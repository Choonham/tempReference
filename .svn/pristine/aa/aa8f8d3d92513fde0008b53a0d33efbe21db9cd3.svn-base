...
const [GET_TEST_REPORT, GET_TEST_REPORT_SECCESS, GET_TEST_REPORT_FAILURE] = createRequestActionTypes('testResultState/GET_TEST_REPORT');
const [GET_STATISTIC_CNT_DATA, GET_STATISTIC_CNT_DATA_SUCCESS, GET_STATISTIC_CNT_DATA_FAILURE] = createRequestActionTypes('testResultState/GET_STATISTIC_CNT_DATA');
const [GET_STATISTIC_DETAIL, GET_STATISTIC_DETAIL_SUCCESS, GET_STATISTIC_DETAIL_FAILURE] = createRequestActionTypes('testResultState/GET_STATISTIC_DETAIL');
const [DELETE_TEST_INFO, DELETE_TEST_INFO_SUCCESS, DELETE_TEST_INFO_FAILURE] = createRequestActionTypes('testResultState/DELETE_TEST_INFO');
...
export const deleteTestInfo = (testID, callbacks)  => {
    return createRequestAction(DELETE_TEST_INFO, testID, callbacks);
};
...
export const getTestReport = (testID, callbacks) => {
    return createRequestAction(GET_TEST_REPORT, testID, callbacks);
};

export const getStatisticCntData = (startDate, endDate, callbacks) => {
    return createRequestAction(GET_STATISTIC_CNT_DATA, {startDate, endDate}, callbacks);
};

export const getStatisticDetail = (startDate, endDate, simulatorIDs, callbacks) => {
    return createRequestAction(GET_STATISTIC_DETAIL, {startDate, endDate, simulatorIDs}, callbacks);
};
...
const getTestReportSaga = createRequestSaga(GET_TEST_REPORT, api.getTestReport);
const getStatisticCntDataSaga = createRequestSaga(GET_STATISTIC_CNT_DATA, api.getStatisticCntData);
const getStatisticDetailSaga = createRequestSaga(GET_STATISTIC_DETAIL, api.getStatisticDetail);
const deleteTestInfoSaga = createRequestSaga(DELETE_TEST_INFO, api.deleteTestInfo);
...
export function* testResultSaga() {
    ...
    yield takeEvery(GET_TEST_REPORT, getTestReportSaga);
    yield takeEvery(GET_STATISTIC_CNT_DATA, getStatisticCntDataSaga);
    yield takeEvery(GET_STATISTIC_DETAIL, getStatisticDetailSaga);
    yield takeEvery(DELETE_TEST_INFO, deleteTestInfoSaga);
}
...
const testResultState = handleActions(
    {
        ...
        [DELETE_TEST_INFO]: (state) => {
            return state;
        },
        [DELETE_TEST_INFO_SUCCESS]: (state, {payload, requestPayload}) => {
            return {
                ...state,
                testResults: state.testResults.filter((e) => {
                    return e.testID !== requestPayload;
                }),
